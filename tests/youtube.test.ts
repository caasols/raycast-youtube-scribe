...
describe("youtube helpers", () => {
  ...
  it("normalizes supported video inputs to canonical watch urls", () => {
    ...
    expect(
      normalizeYoutubeVideoUrl("https://youtu.be/dQw4w9WgXcQ?si=abc"),
    ).toBe("https://www.youtube.com/watch?v=dQw4w9WgXcQ");
    expect(
  ...
  it("detects shorts distinctly from regular videos", () => {
    ...
    expect(detectYoutubeContentKind("https://youtu.be/dQw4w9WgXcQ")).toBe(
      "video",
    );
    expect(
  ...
  it("rejects non-youtube input", () => {
    ...
    ).toThrow("Please provide a valid YouTube URL or video ID.");
  });

  it("extracts the first supported youtube video url from clipboard text", () => {
    expect(
      extractYoutubeUrlFromText("notes https://youtu.be/dQw4w9WgXcQ extra"),
    ).toBe("https://www.youtube.com/watch?v=dQw4w9WgXcQ");
    expect(
      extractYoutubeUrlFromText(
    ...
    expect(
      extractYoutubeUrlFromText("https://www.youtube.com/@veritasium"),
    ).toBeNull();
    expect(extractYoutubeUrlFromText("https://www.youtube.com/")).toBeNull();
    expect(
      extractYoutubeUrlFromText(
        "https://www.youtube.com/@AI_In_Context/videos",
      ),
    ).toBeNull();
  });
