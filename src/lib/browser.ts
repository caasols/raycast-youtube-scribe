...
export function getFocusedYoutubeUrl(): {
  ...
  if (
    !focused.url.includes("youtube.com") &&
    !focused.url.includes("youtu.be")
  ) {
    throw new Error(
      `Focused tab in ${focused.app} is not a YouTube page. Paste a YouTube URL or focus a YouTube tab.`,
    );
  }
